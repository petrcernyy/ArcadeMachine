classdef TetrisClass < handle
    ...
    methods (Access = public)
        function this = TetrisClass(position, windowXLimit, windowYLimit, xLimit, yLimit, updateFrequency, keys)
            % assign parameters to variables
            this.windowXLimit = windowXLimit;
            this.windowYLimit = windowYLimit;
            this.xLimit = xLimit;
            this.yLimit = yLimit;
            ...
            "NumberTitle", "off",...
            "Units", "pixels",...
            "Position", [position, this.windowXLimit, this.windowYLimit],...
            "KeyPressFcn", @this.KeyPressFcnEventHandler...
            );
            ...
            this.fig,...
            "Units", "pixels",...
            "Position", [0, 0, this.windowXLimit, this.windowYLimit],...
            "XLim", [0, this.xLimit + this.leftLimit],...
            "YLim", [0, this.yLimit + this.bottomLimit],...
            "XTick", [],...
            "YTick", []...
            );
            this.ax.XAxis.Color = "none";
            this.ax.YAxis.Color = "none";
            ...
            % create timer
            this.t = timer(...
            "TimerFcn", @this.TimerFcnEventHandler,...
            "period", round(1 / this.updateFrequency, 3),...
            ...
            % start timer -> start the game
            start(this.t);
        end
    end
    methods (Access = private)
        % timer function, main function that handles regular playing field updates
        function TimerFcnEventHandler(this, eventObject, eventArgs)
        ...
        function KeyPressFcnEventHandler(this, eventObject, eventArgs)
            % check whether valid parameters have been passed
            if isempty(this.currentBlock) || isempty(this)
                return;
            end

            % always check whether the move is allowed
            % move down
            if strcmp(eventArgs.Key, this.keys(1))
            ...
            % drop down to bottom
            elseif strcmp(eventArgs.Key, this.keys(2))
                ...
                this.currentBlock.UpdateGraphics();
            % move left
            elseif strcmp(eventArgs.Key, this.keys(3))
            ...
            % move right
            elseif strcmp(eventArgs.Key, this.keys(4))
            ...
            % rotate CW
            elseif strcmp(eventArgs.Key, this.keys(5))
            ...
            % rotate CCW
            elseif strcmp(eventArgs.Key, this.keys(6))
                newCoordinates = this.currentBlock.GetRotatedBlockPosition([-1, 1]);
                if ~(this.CheckBlockPosition(newCoordinates, this.takenPosition, [0, 0]) || this.CheckBlockPosition(this.currentBlock.position(:, 1), this.rightLimit, 0) || this.CheckBlockPosition(this.currentBlock.position(:, 1), this.leftLimit, 0) || this.CheckBlockPosition(this.currentBlock.position, this.takenPosition, [0, -1]) || this.CheckBlockPosition(this.currentBlock.position(:, 2), this.bottomLimit, 0))
                    this.currentBlock.position = newCoordinates;
                    this.currentBlock.UpdateGraphics();
